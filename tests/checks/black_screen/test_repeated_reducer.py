...
def test_repeated_alert_preserves_non_contiguous_segment_evidence():
    _, reduction = apply_records(
        evidenced_record("event-1", 10.0, 100),
        evidenced_record("event-2", 20.0, 105),
        evidenced_record("event-3", 30.0, 110),
    )
...
def test_duplicate_event_does_not_increment_open_incident():
    instance, opened = apply_records(
        record("event-1", 10.0),
        record("event-2", 20.0),
        record("event-3", 30.0),
...
def test_recovery_clears_consumed_history_no_reuse_by_e4():
    instance, opened = apply_records(
        record("event-1", 10.0, 4.0),
        record("event-2", 20.0, 4.0),
        record("event-3", 30.0, 4.0),
    )
    assert opened.alert.state == RepeatedAlertState.OPEN
    assert opened.state.incident.occurrences == 3

    # Recovery occurs and resolves the incident
    recovered = instance.resolve_incident(
        state=opened.state,
        reason="healthy_recovery_confirmed",
    )
    assert recovered.alert.state == RepeatedAlertState.RESOLVED
    assert recovered.clear_state is True
    assert recovered.state.history == ()
    assert recovered.state.incident is None

    # New candidate e4 arrives after recovery
    e4_reduction = instance.record_short_event(
        state=recovered.state,
        record=record("event-4", 50.0, 4.0),
    )
    # Must NOT reopen repeated incident using e2, e3!
    assert e4_reduction.alert is None
    assert e4_reduction.state.incident is None
    assert [item.event_id for item in e4_reduction.state.history] == ["event-4"]
